        ...
        compilation.chunks.forEach((chunk) => {
          chunk.files.forEach((filename) => {
            ...
            if (path.extname(filename) === '.js') {
              // When using the default devtool, the code is inside an eval block with \\n instead
              // of new lines, so reformat to allow doctrine to parse.
              const formattedFileContent = compilation.assets[filename]
                .source()
                .replace(/\\n/g, '\n');

              // Parse the jsdoc and update the specification.
              const parsedFile = parseApiFileContent(formattedFileContent, '.js');
