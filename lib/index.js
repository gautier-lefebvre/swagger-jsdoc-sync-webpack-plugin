...
module.exports = class SwaggerJsdocSyncWebpackPlugin {
   ...
   * @param {boolean} [opts.emitWarningOnError] - Should not prevent emitting the bundle on error.
  ...
  apply(compiler) {
    ...
    compiler.hooks.shouldEmit.tap(
      ...
      (compilation) => {
        if (lastError) {
          if (this.opts.emitWarningOnError) {
            compilation.warnings.push(lastError);
          } else {
            compilation.errors.push(lastError);
          }

          lastError = null;
          return !this.opts.emitWarningOnError;
        }
